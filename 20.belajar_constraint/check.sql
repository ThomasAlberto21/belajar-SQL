...
VALUES ("P0016", "Permen", 500, "10");

-- * Check berfungsi untuk mengecek apakah data yang diinput layak untuk dimasukkan ke dalam database sesuai kriteria yang kita inginkan

-- ! Error karena ada data price yang di bawah 1000

ALTER TABLE products ADD CONSTRAINT check_price CHECK(price >= 1000);

UPDATE products SET price = 1000 WHERE id = "P0016";

-- * Tidak error lagi karena data yang diinput sesuai kriteria yang kita inginkan
ALTER TABLE products ADD CONSTRAINT check_price CHECK(price >= 1000);
