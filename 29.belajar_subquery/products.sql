...
SELECT
    COUNT(id) AS "Banyak Data Di MySQL",
    category
FROM products
GROUP BY category;
...
SELECT
    SUM(quantity) AS "Banyak Stok Barang",
    category
FROM products
GROUP BY category;
...
ALTER TABLE products
ADD
    FULLTEXT products_fulltext(name, description);

SHOW CREATE TABLE products;

-- Mencari kata ayam yang terdapat di dalam database
...
-- Mencari kata ayam dan mengabaikan kata bakso di dalam database
...
-- Mencari kata bakso dan mencari kemiripan dengan data bakso di dalam database
...
-- Tambah Data Untuk Table Relationship
...
VALUES ("X0001", "X Satu", 2000, 5), ("X0002", "X Dua", 2000, 5), ("X0003", "X Tiga", 2000, 5);
...
UPDATE products SET price = 100000 WHERE id = "X0001";

SELECT * FROM products;

-- Todo Subquery Untuk Mencari harga Tertinggi Yang Mempunyai  Kategori
